...
**注记1** 方向导数的主要用法都是在几何学和三维物理学中得知的. 不过，这些概念在二维中也可以定义，而且具备相似（但更简约）的性质. 因此，曲线$f(x,y)=c_0$可以被看作函数$z=f(x,y)$的一条等值线；而且如果这个函数的梯度被定义为
...
$$\grad f=\left(\frac{\partial}{\partial x}\mathbf{i}+\frac{\partial}{\partial y}\mathbf{j}+\frac{\partial}{\partial k}\mathbf{k}\right)f$$

函数$f$前面的**del算子**一般记作符号$\nabla$（一个反转的delta，读作「del」）[^9]，也即

$$\nabla=\frac{\partial}{\partial x}\mathbf{i}+\frac{\partial}{\partial y}\mathbf{j}+\frac{\partial}{\partial k}\mathbf{k}$$
